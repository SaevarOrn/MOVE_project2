...
(\textit{Since the robot must make a decision based only on location and orientation, it is sufficient to show that the robot completes a single complete cycle.
A deterministic strategy must therefore be used to ensure that all states are visited infinitely often.
It is possible to ensure that if the robot has a winning strategy, it will finish in the starting position.
This is done by checking whether the robot can ever be in a state where it has visited $(0,0)$ but not visited every state:
$A<>(!origin\_incomplete())$ where $origin\_incomplete()$ returns true IFF $visited[0][0]$ \&\& $!all\_visited()$
})
